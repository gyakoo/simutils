...
{
  static const char* names[FLT_NODE_MAX]={"none","base","xref","group","object","mesh","lod","face", "vertex_list", "switch"};
  if ( !n ) return;
...
void fltXmlOfPrint(flt* of, int d, std::set<uint64_t>& done, int* tot_nodes, fltThreadPool* tp)
{
  ...
  if ( of->header )
  {
    ...
    fltXmlIndent(d+2); printf( "<v_coords_units>%d</v_coords_units>\n", of->header->v_coords_units );
    fltXmlIndent(d+2); printf( "<flags>0x%08x</flags>\n", of->header->flags );
    fltXmlIndent(d+2); printf( "<proj_type>%d</proj_type>\n", of->header->proj_type );
    fltXmlIndent(d+2); printf( "<db_orig>%d</db_orig>\n", of->header->db_orig );
    ...
    fltXmlIndent(d+2); printf( "<earth_ellip_model>%d</earth_ellip_model>\n", of->header->earth_ellip_model );
...
bool fltReadImageAttr(const char* filename, fltImgAttr* attr)
{
  ...
  else if ( fltExtensionIs(filename, ".dds") )
  {
    ...
    {
      if ( allm==0xffffffff ) attr->depth = 32;
      else if ( allm==0x00ffffff || allm==0xffffff00 ) attr->depth=24;
      else if ( allm==0xff || allm==0xff000000 ) attr->depth=8;
    }
  }
